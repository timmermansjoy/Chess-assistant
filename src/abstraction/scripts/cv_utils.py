...
def get_squares(corners, img):
    ...
    # Draw outer points
    cv2.line(img, (xtl, ytl), (xtr, ytr), RED, Line_thickness)
    cv2.line(img, (xtl, ytl), (xbl, ybl), RED, Line_thickness)
    cv2.line(img, (xbl, ybl), (xbr, ybr), RED, Line_thickness)
    cv2.line(img, (xbr, ybr), (xtr, ytr), RED, Line_thickness)
    for i in range(len(corners)):
        ...
        yl = (corners[i][0][1])
        cv2.line(img, (xl, yl), (corners[i][0][0], corners[i][0][1]), RED, Line_thickness)

        # Right outer line
        xr = corners[i][6][0] + (corners[i][6][0] - corners[i][5][0])
        yr = (corners[i][6][1])
        cv2.line(img, (corners[i][6][0], corners[i][6][1]), (xr, yr), RED, Line_thickness)

        # top outer line
        xt = corners[0][i][0]
        yt = corners[0][i][1] - (corners[1][i][1] - corners[0][i][1])
        cv2.line(img, (corners[0][i][0], corners[0][i][1]), (xt, yt), RED, Line_thickness)

        # bottom outer line
        xb = corners[6][i][0]
        yb = corners[6][i][1] + (corners[6][i][1] - corners[5][i][1])
        cv2.line(img, (corners[6][i][0], corners[6][i][1]), (xb, yb), RED, Line_thickness)

        # B ttomouter line

        for j in range(len(corners[i])):
            if begin_vertical is not None:
                cv2.line(img, (begin_vertical[0], begin_vertical[1]), (corners[i][j][0], corners[i][j][1]), RED, Line_thickness)
                cv2.line(img, (begin_horizontal[0], begin_horizontal[1]), (corners[j][i][0], corners[j][i][1]), RED, Line_thickness)
            begin_vertical = corners[i][j]
            begin_horizontal = corners[j][i]

    #img = crop_on_board(img, corners)
    return img
